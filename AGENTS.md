...
## Google Play Console Browser Rule

- For Play Console work, first use the already authenticated in-app browser session available to Codex.
- Do not assume that the user changed Chrome or needs a browser extension. Confirm the active browser session and console state before diagnosing any access issue.
- Continue upload, release creation, and review submission in that connected session whenever it is available; do not send the user through unnecessary browser setup steps.
- When the user says Chrome is already open and logged in, open the required Play Console URL in that existing Chrome session and continue the task directly. Do not stop at a missing API permission or missing remote-debugging connection, and do not ask the user to reinstall, reconnect, or configure Chrome unless the actual Console page itself requires a new login or explicit user approval.
- Before reporting a Play Console task as blocked, attempt the normal interactive Chrome path first. State the exact Console page and error only if that direct attempt fails.
