...
if [ -n "${DB_CONTAINER_NAME:-}" ]; then
  ...
  password_hash="$(docker exec "$DB_CONTAINER_NAME" psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -tAc "select coalesce(password_hash, '') from app_users where id = $user_id")"
  if [ -z "$password_hash" ] || [ "$password_hash" = "$password" ]; then
    echo "Password was not stored as a hash" >&2
    exit 1
  fi
  case "$password_hash" in
    \$2a\$*|\$2b\$*|\$2y\$*) ;;
    *)
      echo "Password hash is not a bcrypt hash: $password_hash" >&2
      exit 1
      ;;
  esac
...
api_expect_status 429 POST /community/posts '{"boardType":"free","title":"Rate limited post","body":"body","mediaUrls":[]}'
