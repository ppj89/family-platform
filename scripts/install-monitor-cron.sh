...
$MONITOR_TIME root cd "$APP_DIR" && mkdir -p logs && BASE_URL="$BASE_URL" DISK_WARN_PERCENT="$DISK_WARN_PERCENT" ALERT_WEBHOOK_URL="$ALERT_WEBHOOK_URL" scripts/monitor-prod.sh >> logs/monitor.log 2>&1
EOF
