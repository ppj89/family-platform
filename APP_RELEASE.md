...
- Android app backup is disabled in the release manifest so local session and family data are not copied into device backups.
- Keep app advertising disabled until production ad IDs, consent handling, privacy policy text, and `app-ads.txt` are ready.
