...
export default function RestaurantPage() {
  ...
  async function reload() {
    ...
    try {
      ...
      setMessage(apiActionMessage(error, '맛집 목록을 불러오지 못했습니다.'))
    } finally {
      setLoading(false)
    }
  }
  ...
  function startEdit(item: RestaurantItem) {
    ...
    setForm({
      ...
      scope: item.scope || scopeOptions[0],
      memo: item.memo || '',
  ...
  async function confirmSave() {
    ...
    try {
      const payload: RestaurantPayload = {
        ...
        scope: form.scope || scopeOptions[0],
      ...
      await reload()
      resetForm()
      setMessage(editing ? '맛집을 수정했습니다.' : '맛집을 추가했습니다.')
    } catch (error) {
      setMessage(apiActionMessage(error, editing ? '맛집 수정에 실패했습니다.' : '맛집 추가에 실패했습니다.'))
    } finally {
      setConfirmKind(null)
      setLoading(false)
    }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setMessage('맛집을 삭제했습니다.')
    } catch (error) {
      setMessage(apiActionMessage(error, '맛집 삭제에 실패했습니다.'))
    } finally {
      setConfirmKind(null)
  ...
  return (
    <section className="fp-restaurant fp-card">
      {loading ? <div className="fp-loading-blocker">처리 중</div> : null}
      <header className="fp-restaurant-header">
        <div>
          <h2>맛집</h2>
          <p>{restaurantList.length}건</p>
        </div>
      </header>
      {message ? <p className="fp-message">{message}</p> : null}
      <div className="fp-restaurant-layout">
        <section className="fp-restaurant-list">
          {restaurantList.length ? restaurantList.map((item) => (
            <article className="fp-restaurant-row" key={item.id}>
              <div className="fp-restaurant-row-main">
                <time>{item.visitDate}</time>
                <strong>{item.name}</strong>
                <p>{[item.menu, item.location || item.address].filter(Boolean).join(' · ') || '상세 정보 없음'}</p>
                <small>{[money(item.price), item.rating ? `${item.rating}점` : '', item.scope].filter(Boolean).join(' · ')}</small>
              </div>
        ...
        <form className="fp-restaurant-form fp-card" onSubmit={requestSave}>
          ...
          <div className="fp-form-grid">
            ...
            <label className="fp-field">
              가격
              <select value={form.price ?? ''} onChange={(event) => setForm((value) => ({ ...value, price: numberOrNull(event.target.value) }))}>
                {priceOptions.map((option) => <option key={option.label} value={option.value}>{option.label}</option>)}
              </select>
            </label>
            <label className="fp-field">
              별점
              <select value={form.rating ?? ''} onChange={(event) => setForm((value) => ({ ...value, rating: numberOrNull(event.target.value) }))}>
            ...
            <label className="fp-field">
              공개범위
              <select value={form.scope || scopeOptions[0]} onChange={(event) => setForm((value) => ({ ...value, scope: event.target.value }))}>
          ...
          <button className="fp-button fp-button-primary" type="submit">{editing ? '수정' : '추가'}</button>
        </form>
      </div>

      {confirmKind ? (
        <ConfirmDialog
          title={confirmKind === 'delete' ? '맛집을 삭제할까요?' : editing ? '맛집을 수정할까요?' : '맛집을 추가할까요?'}
          body={confirmKind === 'delete' ? '선택한 맛집을 삭제합니다.' : '입력한 내용을 맛집 목록에 반영합니다.'}
