...
function resolveStartUrl(provider: SsoProvider, configured?: SsoProviderInfo) {
  ...
  if (startUrl) {
    ...
    if (startUrl.startsWith('/')) {
      ...
      if (startUrl === apiUrl.pathname || startUrl.startsWith(`${apiUrl.pathname}/`)) {
        return apiBaseUrl.startsWith('http') ? `${apiUrl.origin}${startUrl}` : startUrl
      }
    }
    return `${apiBaseUrl}${startUrl.startsWith('/') ? startUrl : `/${startUrl}`}`
  }
...
function isEmailVerificationRequired(error: unknown) {
  const status = typeof error === 'object' && error && 'status' in error ? Number((error as ApiError).status) : 0
  const message = error instanceof Error ? String(error.message || '').toLowerCase() : ''
  return status === 403 && (message.includes('email verification') || message.includes('이메일 인증'))
}

function isVerificationResendLimitReached(error: unknown) {
  const status = typeof error === 'object' && error && 'status' in error ? Number((error as ApiError).status) : 0
  const message = error instanceof Error ? String(error.message || '').toLowerCase() : ''
  return status === 429 && message.includes('verification email resend limit')
}
...
export function LoginPage() {
  ...
  const [verificationResendBusy, setVerificationResendBusy] = useState(false)
  const [verificationResendCooldown, setVerificationResendCooldown] = useState(0)
  const [verificationResendsRemaining, setVerificationResendsRemaining] = useState(2)
  ...
  function resetMode(nextMode: AuthMode) {
    ...
    setVerificationResendsRemaining(2)
  }

  async function resendVerificationEmail() {
    const email = verificationEmail.trim()
    if (!email || verificationResendBusy || verificationResendCooldown > 0) {
      return
    }
    setVerificationResendBusy(true)
    try {
      const response = await apiRequest<VerificationResendResponse>('/auth/verification/resend', {
        method: 'POST',
        body: { email },
      })
      const remaining = Math.max(0, Math.min(2, Number(response.remainingResends ?? verificationResendsRemaining - 1)))
      setVerificationResendsRemaining(remaining)
      setVerificationResendCooldown(60)
      showToast(remaining > 0 ? `인증 메일을 다시 보냈습니다. 재전송은 ${remaining}회 더 가능합니다.` : '인증 메일을 다시 보냈습니다. 이번이 마지막 재전송입니다.')
    } catch (error) {
      if (isVerificationResendLimitReached(error)) {
        setVerificationResendsRemaining(0)
        showToast('인증 메일 재전송은 회원가입 후 2회까지만 가능합니다.')
      } else {
        showToast(getErrorMessage(error))
      }
    } finally {
      setVerificationResendBusy(false)
    }
  }
  ...
  async function submitAuth(forceLogin = false) {
    ...
    if (mode === 'register') {
      if (verificationEmail) {
        showToast('현재 회원가입 진행 중인 계정의 이메일 인증을 먼저 완료해주세요.')
        return
      }
    ...
    try {
      const response = await apiRequest<AuthSessionResponse>(mode === 'login' ? '/auth/login' : '/auth/register', {
        ...
        body:
          mode === 'login'
            ? { email: loginForm.email.trim(), password: loginForm.password, forceLogin, autoLogin }
            : {
      ...
      if (mode === 'register' && response.emailVerificationRequired) {
        ...
        setVerificationResendsRemaining(Math.max(0, Math.min(2, Number(response.verificationResendsRemaining ?? 2))))
        setVerificationResendCooldown(0)
        return
      }
      ...
      // App.tsx skips the immediately following reload because the login API
      // has already recorded it. Keep this short-lived so an interrupted app
      // launch cannot hide a later automatic-login history record.
      window.sessionStorage.setItem('family-platform-login-just-completed', String(Date.now()))
      window.location.reload()
    } catch (error) {
      if (mode === 'login' && !forceLogin && isDuplicateSession(error)) {
        setSessionConfirmOpen(true)
      } else if (mode === 'login' && isEmailVerificationRequired(error)) {
        setMode('register')
        setVerificationEmail(loginForm.email.trim())
        setRegisterForm((current) => ({ ...current, email: loginForm.email.trim() }))
        setVerificationResendsRemaining(2)
        showToast('회원가입 탭에서 진행 중인 계정의 인증 메일을 다시 보낼 수 있습니다.')
      } else {
        showToast(getErrorMessage(error))
      }
    } finally {
      ...
      <section className="auth-layout">
        <div className="auth-visual" data-auth-landing-ready="true">
          ...
          <section className="auth-copy">
            <span className="auth-kicker">커플, 지인을 위한 하나의 공간</span>
            <p>일정, 가계부, 여행, 육아, 일기를 사람들과 메뉴별로 공유하고 관리합니다.</p>
          </section>
        </div>
        ...
        <form className="auth-card" data-auth-mode={mode} onSubmit={handleSubmit}>
          ...
          {mode === 'login' ? (
            ...
            <RegisterFields
              ...
              disabled={Boolean(verificationEmail)}
          ...
          {mode === 'register' && verificationEmail ? (
            <section className="auth-verification-resend" aria-live="polite">
              <div>
                <strong>인증 메일을 받지 못했나요?</strong>
                <p><b>{verificationEmail}</b> 계정의 회원가입이 진행 중입니다.</p>
              </div>
            </section>
          ) : null}

          {mode === 'register' ? (
            <div className={`auth-submit-row${verificationEmail ? '' : ' single'}`}>
              {verificationEmail ? (
                <button className="auth-verification-resend-button" type="button" onClick={() => void resendVerificationEmail()} disabled={verificationResendBusy || verificationResendCooldown > 0 || verificationResendsRemaining <= 0}>
                  {verificationResendBusy ? '재전송 중...' : verificationResendsRemaining <= 0 ? '재전송 횟수 사용 완료' : verificationResendCooldown > 0 ? `${verificationResendCooldown}초 후 재전송` : `인증메일 재전송 (${verificationResendsRemaining}회)`}
                </button>
              ) : null}
              <button className="auth-submit" type="submit" disabled={busy || Boolean(verificationEmail)}>
                {busy ? '처리 중' : verificationEmail ? '인증 대기 중' : '회원가입'}
              </button>
            </div>
          ) : (
