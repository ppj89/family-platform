...
import { getKoreanHolidayName } from '../../../shared/utils/koreanHolidays'
...
import { requestTodayCalendarNotificationRefresh } from '../notifications/useTodayCalendarNotifications'
...
// v3 drops positions that were accidentally persisted while Android's
// software keyboard had reduced the viewport height. New drag positions are
// preserved, but the old mid-screen values must not be restored.
const quickNavPositionKey = 'family-platform-calendar-quick-nav-position-v3'
...
function CalendarNavChevron({ direction }: { direction: 'previous' | 'next' }) {
  return direction === 'previous'
    ? <CgChevronLeft className="fp-calendar-nav-chevron" aria-hidden="true" />
    : <CgChevronRight className="fp-calendar-nav-chevron" aria-hidden="true" />
}
...
export default function CalendarPage() {
  ...
  const [scheduleSearchItems, setScheduleSearchItems] = useState<ScheduleItem[]>([])
  const [isScheduleSearchLoading, setIsScheduleSearchLoading] = useState(false)
  const [scheduleSearchMessage, setScheduleSearchMessage] = useState('')
  ...
  const [isPastMonthAgendaExpanded, setIsPastMonthAgendaExpanded] = useState(false)
  ...
  const holidayRange = useMemo(() => {
    if (view === 'month' && cells.length) {
      return { startDate: cells[0].dateKey, endDate: cells[cells.length - 1].dateKey }
    }
    return range
  }, [cells, range, view])
  ...
  const pastMonthAgendaGroups = useMemo(
    () => monthAgendaGroups.filter((group) => group.dateKey < today),
    [monthAgendaGroups, today],
  )
  const currentAndFutureMonthAgendaGroups = useMemo(
    () => monthAgendaGroups.filter((group) => group.dateKey >= today),
    [monthAgendaGroups, today],
  )
  const pastMonthAgendaItemCount = useMemo(
    () => pastMonthAgendaGroups.reduce((count, group) => count + group.items.length, 0),
    [pastMonthAgendaGroups],
  )
  ...
  const scheduleSearchResults = useMemo(() => {
    const keyword = scheduleSearchQuery.trim().toLocaleLowerCase('ko-KR')
    if (!keyword) return []
    return scheduleSearchItems
      .filter((item) => [item.title, item.category, item.memberName, item.memo].filter(Boolean).join(' ').toLocaleLowerCase('ko-KR').includes(keyword))
      .sort((a, b) => `${a.scheduleDate} ${a.scheduleTime || ''}`.localeCompare(`${b.scheduleDate} ${b.scheduleTime || ''}`))
  }, [scheduleSearchItems, scheduleSearchQuery])
  const scheduleSearchGroups = useMemo(() => {
    const groups = new Map<string, ScheduleItem[]>()
    scheduleSearchResults.forEach((item) => {
      const group = groups.get(item.scheduleDate)
      if (group) group.push(item)
      else groups.set(item.scheduleDate, [item])
    })
    return Array.from(groups.entries()).map(([date, groupItems]) => ({ date, items: groupItems }))
  }, [scheduleSearchResults])

  function getHolidayName(dateKey: string) {
    return holidayNames[dateKey] || getKoreanHolidayName(dateKey)
  }
  ...
  function focusToday() {
    syncSelectedDate(todayKey(), { yearSelected: view === 'year' })
    setIsQuickNavOpen(false)
    window.requestAnimationFrame(scrollCalendarTop)
  }
  ...
  async function openScheduleSearch() {
    setIsScheduleSearchOpen(true)
    setScheduleSearchQuery('')
    setScheduleSearchMessage('')
    setIsScheduleSearchLoading(true)
    try {
      setScheduleSearchItems(await listSchedules(scheduleStartDate, '2100-12-31'))
    } catch (error) {
      if (!isAuthError(error)) setScheduleSearchMessage(apiActionMessage(error, '일정을 불러오지 못했습니다.'))
    } finally {
      setIsScheduleSearchLoading(false)
      window.setTimeout(() => scheduleSearchInputRef.current?.focus(), 0)
    }
  }

  function closeScheduleSearch() {
    setIsScheduleSearchOpen(false)
    setScheduleSearchMessage('')
  }

  function openScheduleSearchResult(item: ScheduleItem) {
    closeScheduleSearch()
    setScheduleDetail({
      ...item,
      instanceKey: `${item.id}-${item.scheduleDate}`,
      occurrenceDate: item.scheduleDate,
  ...
  useEffect(() => {
    setIsPastMonthAgendaExpanded(false)
  }, [monthDate, view])
  ...
  useEffect(() => {
    ...
    async function loadHolidays() {
      try {
        const holidays = await listHolidays(holidayRange.startDate, holidayRange.endDate)
        if (cancelled) return
        setHolidayNames((current) => {
          const next = { ...current }
          holidays.forEach((holiday) => {
            next[holiday.dateKey] = holiday.name
          })
          return next
        })
      } catch (error) {
  ...
  }, [holidayRange.endDate, holidayRange.startDate])

  useEffect(() => {
    function clampOnResize() {
      const viewportWidth = window.innerWidth
      // The software keyboard only changes the visible height.  Never clamp or
      // persist a drag position in that state or the FAB remains mid-screen.
  ...
  function requestSave(event: FormEvent) {
    ...
    if (!isCompleteTime(form.scheduleTime)) {
      window.setTimeout(() => formRef.current?.querySelector<HTMLInputElement>('[data-required-field="schedule-time"]')?.focus({ preventScroll: true }), 0)
  ...
  async function saveSchedule() {
    ...
    try {
      ...
      requestTodayCalendarNotificationRefresh()
  ...
  function renderMonthAgendaGroup(group: { dateKey: string; items: CalendarScheduleInstance[] }, isPast = false) {
    const firstItem = group.items[0]
    if (!firstItem) return null
    const hiddenCount = group.items.length - 1
    return (
      <article
        className={`fp-month-agenda-row${isPast ? ' past' : ''}`}
        key={group.dateKey}
        role="button"
        tabIndex={0}
      ...
      >
        ...
        <div className="fp-month-agenda-main">
          <strong className="fp-ellipsis" title={firstItem.title}>{firstItem.title}</strong>
          <p>
    ...
    <>
      <section className="fp-calendar content-grid">
        ...
        <article className="panel wide family-calendar-panel fp-calendar-left" ref={calendarTopRef}>
          <section className="fp-calendar-left-card">
            <header className="fp-calendar-header">
              <div className="fp-calendar-actions calendar-toolbar">
                ...
                <div className="fp-calendar-action-buttons">
                  <button className="fp-calendar-search-button" type="button" onClick={() => void openScheduleSearch()}>조회</button>
                  <button
            ...
            <div className="fp-month-grid family-month-grid">
              ...
              {cells.map((cell) => {
                ...
                const holidayName = cell.dateKey ? getHolidayName(cell.dateKey) : ''
                ...
                return (
                  ...
                  >
                    ...
                    {dayItems.slice(0, 1).map((item) => (
                      ...
                      >
                        <span className="fp-ellipsis" title={item.title}>{item.title}</span>
                      </button>
                    ))}
            ...
            <div className="fp-schedule-list">
              {view === 'month'
                ? monthAgendaGroups.length
                  ? <>
                      {pastMonthAgendaGroups.length ? (
                        <div className="fp-month-agenda-past">
                          <button
                            type="button"
                            className="fp-month-agenda-toggle"
                            aria-expanded={isPastMonthAgendaExpanded}
                            aria-controls="fp-month-past-agenda"
                            onClick={() => setIsPastMonthAgendaExpanded((current) => !current)}
                          >
                            <span>이전 일정</span>
                            <small>{pastMonthAgendaItemCount}건</small>
                            {isPastMonthAgendaExpanded ? <HiChevronUp aria-hidden="true" /> : <HiChevronDown aria-hidden="true" />}
                          </button>
                          {isPastMonthAgendaExpanded ? (
                            <div id="fp-month-past-agenda" className="fp-month-agenda-past-list">
                              {pastMonthAgendaGroups.map((group) => renderMonthAgendaGroup(group, true))}
                            </div>
                          ) : null}
                        </div>
                      ) : null}
                      {currentAndFutureMonthAgendaGroups.map((group) => renderMonthAgendaGroup(group))}
                    </>
          ...
          {!isQuickNavKeyboardOpen && !isFormDialogOpen ? <nav
          ...
          >
            {isQuickNavOpen ? (
              <div className="fp-calendar-scroll-menu">
                ...
                <button className="fp-calendar-scroll-text-action" type="button" onClick={focusToday}>오늘</button>
                <button className="fp-calendar-scroll-text-action" type="button" onClick={() => startCreate(selectedDate, { focus: true })}>입력</button>
              </div>
            ) : null}
            <button
              type="button"
          ...
          {isFormDialogOpen ? (
            ...
            <header>
              ...
              <button type="button" className="fp-calendar-form-close" aria-label="닫기" onClick={closeFormDialog}>
                <HiOutlineX aria-hidden="true" />
              </button>
            </header>
        <div className="fp-calendar-form-scroll">
        <div className="fp-form-grid">
          ...
          <label className="fp-field">
            ...
            <input
              ...
              maxLength={5}
              placeholder="HH:mm"
              data-required-field="schedule-time"
          ...
          <CalendarCustomSelect
            label="일정 시간 알림"
            value={form.pushEnabled ? 'enabled' : 'disabled'}
            options={[
              { label: '설정', value: 'enabled' },
              { label: '미설정', value: 'disabled' },
            ]}
            onChange={(value) => setForm({ ...form, pushEnabled: value === 'enabled' })}
          />
          <label className="fp-field span-2">
            <span>메모</span>
            ...
            <div className="fp-dialog-action-footer">
              <button className="fp-button fp-button-muted" type="button" onClick={closeFormDialog}>취소</button>
      ...
      {isScheduleSearchOpen ? (
        <div className="fp-calendar-popup-backdrop fp-calendar-search-backdrop" role="presentation" onClick={closeScheduleSearch}>
          <section
            className="fp-calendar-popup fp-calendar-search-popup"
            role="dialog"
            aria-modal="true"
            aria-labelledby="fp-calendar-search-title"
            onClick={(event) => event.stopPropagation()}
          >
            <header>
              <h3 id="fp-calendar-search-title">일정 조회</h3>
              <button type="button" aria-label="닫기" onClick={closeScheduleSearch}>
                <HiOutlineX aria-hidden="true" />
              </button>
            </header>
            <div className="fp-calendar-search-input-wrap">
              <input
                ref={scheduleSearchInputRef}
                type="text"
                placeholder="일정명, 구분, 구성원, 메모"
                value={scheduleSearchQuery}
                onChange={(event) => setScheduleSearchQuery(event.target.value)}
              />
            </div>
            {isScheduleSearchLoading ? <p className="fp-calendar-search-status">일정을 불러오는 중입니다.</p> : null}
            {scheduleSearchMessage ? <p className="fp-calendar-search-status error">{scheduleSearchMessage}</p> : null}
            {!isScheduleSearchLoading && !scheduleSearchMessage && !scheduleSearchQuery.trim() ? <p className="fp-calendar-search-status">검색어를 입력하면 전체 일정에서 조회합니다.</p> : null}
            {!isScheduleSearchLoading && scheduleSearchQuery.trim() ? (
              <div className="fp-calendar-search-results">
                <p className="fp-calendar-search-count">검색 결과 {scheduleSearchResults.length}건</p>
                {scheduleSearchGroups.length ? scheduleSearchGroups.map((group) => (
                  <section className="fp-calendar-search-day-group" key={group.date}>
                    <h4>{formatKoreanDate(group.date)}</h4>
                    {group.items.map((item) => (
                      <article
                        className="fp-calendar-search-row"
                        key={item.id}
                        role="button"
                        tabIndex={0}
                        onClick={() => openScheduleSearchResult(item)}
                        onKeyDown={(event) => {
                          if (event.key !== 'Enter' && event.key !== ' ') return
                          event.preventDefault()
                          openScheduleSearchResult(item)
                        }}
                      >
                        <strong>{scheduleTime(item)}</strong>
                        <div>
                          <b>{item.title}</b>
                          <p>{[item.category || '일정', item.memberName, item.memo].filter(Boolean).join(' · ')}</p>
                        </div>
                      </article>
                    ))}
                  </section>
                )) : <p className="fp-calendar-search-status">검색 결과가 없습니다.</p>}
              </div>
            ) : null}
          </section>
        </div>
        ...
        <div className="fp-calendar-popup-backdrop" role="presentation" onClick={() => setDayDialog(null)}>
          <section className="fp-calendar-popup" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            <header>
              ...
              <button type="button" aria-label="닫기" onClick={() => setDayDialog(null)}>
                <HiOutlineX aria-hidden="true" />
              </button>
            </header>
            <div className="fp-schedule-list">
        ...
        <div className="fp-calendar-popup-backdrop" role="presentation" onClick={() => setMonthDialog(null)}>
          <section className="fp-calendar-popup fp-year-month-popup" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            <header>
              ...
              <button type="button" aria-label="닫기" onClick={() => setMonthDialog(null)}>
                <HiOutlineX aria-hidden="true" />
              </button>
            </header>
      ...
      {scheduleDetail ? (
        <div className="fp-schedule-detail-backdrop" role="presentation" onClick={() => setScheduleDetail(null)}>
          <section className="fp-schedule-detail-dialog" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            <button className="fp-schedule-detail-close" type="button" aria-label="닫기" onClick={() => setScheduleDetail(null)}>
              <HiOutlineX aria-hidden="true" />
            </button>
